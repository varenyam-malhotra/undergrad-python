...
# Varenyam Malhotra
# September 25, 2024
# Project A; Gateway Computing Python

import math

#from simVis1 import visualize

# importing simvis1 manually as above line failed despite same folder
"""
...
# End of the imported simVis1 file

# Beginning of the actual project A python file with methods and code for that methods and docstrings and comments  
...
def updateX(x,vx,dt): 
    ...
    ----------
    x : old x-coordinate
    vx : velocity of object in x-direction
    dt : time step for the update
    Returns
    -------
    xnew : the new, updated x-value 
    """
    xnew = x + (vx * dt)
    return xnew
...
def updateY(y,vy,dt):
    ...
    ----------
    y : old y-coordinate
    vy : velocity of object in y-direction
    dt : time step for the update
    Returns
    -------
    ynew : the new, updated y-value
    """
...
def boxCollision(x,y,vx,vy,xLow,xHigh,yLow,yHigh):
    """
    Checks whether a circle collides with the walls or a box or not

    Parameters
    ----------
    x : x-coordinate of center of circle
    y : y-coordinate of center of circle
    vx : current velocity x-component of circle
    vy : current velocity y-component of circle
    xLow : The left end of the x-coordinate of the box
    xHigh : The right end of the x-coordinate of the box
    yLow :The bottom end of the y-coordinate of the box
    yHigh : The top end of the y-coordinate of the box

    Returns
    -------
    x : updated x-coordinate of circle center
    y : updated y-coordinate of circle center
    vx : velocity x-component of circle
    vy : velocity y-component of circle
...
def get_distance(x1,y1,x2,y2):
    ...
    Returns
    -------
    distance : The eucledian distance between (x1, y1) and (x2, y2)
...
def Overlap(x1,y1,radius1,x2,y2,radius2):
    """
    determines whether two circles overlap/collide
    
    Parameters
    ----------
    x1 : x-coordinate of center for circle 1
    y1 : y-coordinate of center for circle 1
    radius1 : radius of cricle 1
    x2 : x-coordinate of center for circle 2
    y2 : y-coordinate of center for circle 2
    radius2 : radius of circle 2

    Returns
    -------
    bool
        True or False based on whether the circles overlap/collide
...
def get_unit_direction(x1,y1,x2,y2):
    """
    Calculates unit direction vector between two points
    Parameters
    ----------
    ...
    Returns
    -------
    unit direction vector to 3 decimal places for both x-direction and y-direction
    """
...
def dot_product(x1,y1,x2,y2):
    ...
    Returns
    -------
    the dot product of the vector <x1,y2> and <x2,y2> which represents the closeleness of the alignment of the vectors' directions
...
def update_collision_velocity(x1, y1, v1x, v1y, x2, y2, v2x, v2y):
    """
    calculates updated velocity of two objects after a collision based on their initial velocities and positions

    Parameters
    ----------
    x1 : x-coordinate of center of first circle
    y1 : y-coordinate of center of first circle
    v1x : velocity x-component of first circle
    v1y : velocity y-component of first circle
    x2 : x-coordinate of center of second circle
    y2 : y-coordinate of center of second circle
    v2x : velocity x-component of second circle
    v2y : velocity y-component of second circle

    Returns (all rounded to 3rd decimal place)
    -------
    V1x : x-component of velocity of first circle after collision
    V1y : y-component of velocity of first circle after collision
    V2x : x-component of velocity of second circle after collision
    V2y : y-component of velocity of second circle after collision
    """    
...
def circleCollision(x1,y1,radius1,v1x, v1y, x2,y2,radius2,v2x,v2y):
    """
    Simulates the collision and response of the two circles in the 2D box space.
    ...
    v1x : x-component of velocity vector of first circle
    v1y : y-component of velocity vector of first circle
    x2 : x-coordinate of center of second circle
    y2 : y-coordinate of center of second circle
    radius2 : radius of second circle
    v2x : x-coordinate of velocity vector of second circle
    v2y : y-coordinate of velocity vector of second circle
    Returns
    -------
    Returns new positions and velocities of both circles
    """
    ...
    return round(x1_new, 3), round(y1_new, 3), round(v1x_new, 3), round(v1y_new, 3), round(x2_new, 3), round(y2_new, 3), round(v2x_new, 3), round(v2y_new, 3)
